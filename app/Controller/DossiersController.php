...
if($usr['Group']['name']== 'Administrators' && $usr['User']['id']==$currDossier['Dossier']['user_id'])
  {//admin  
        ...
        $this->loadModel('User');
		$newusername=$this->request->data['Dossier']['attribut6'].$this->request->data['Dossier']['attribut5'];
        $newusername=strtolower($newusername);
        $userslike=$this->User->find('all',array('conditions'=>array('LCASE(User.username) LIKE'=>$newusername.'%')));

          $usernames=array();
          foreach($userslike as $user){
array_push($usernames,$user['User']['username']);
}
 
if(in_array($newusername,$usernames)){
$i=1;
$tmpusername=$newusername;
while(in_array($tmpusername,$usernames)){
$tmpusername=$newusername.$i;
++$i;
}
...
//  			
// 			}
 	    return $this->redirect(array('action' =>$this->request->data['redirectionpage'],'?'=>array('dossierid'=>$dossierid)));
     	}
    else	
		{
...
public function editnotes($code)
{
	...
	/*	if($dossiercode['Dossiercode']['valid']==0)||date("Y-m-d h:i:s")>date("Y-m-d h:i:s",strtotime($dossiercode['Dossiercode']['datefin'])*/ /*|| date("Y-m-d h:i:s")<date("Y-m-d h:i:s",strtotime($dossiercode['Dossiercode']['datedebut'])){
		...
		$res=$this->getNomPrenom($dossiercode['Dossiercode']['dossier_id']);
		
		
		$this->set('prenom',$res['nom']);
		$this->set('nom',$res['prenom']);
